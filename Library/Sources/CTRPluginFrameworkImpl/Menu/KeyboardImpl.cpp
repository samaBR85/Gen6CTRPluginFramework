...
namespace CTRPluginFramework {
    ...
    vector<TouchKey> KeyboardImpl::_DecimalKeys;
    vector<TouchKey> KeyboardImpl::_HexaDecimalKeys;
    vector<TouchKey> KeyboardImpl::_QwertyKeys;

    KeyboardImpl::KeyboardImpl(const string &text) {
        _owner = nullptr;
        _text = text;
        _error = "";
        _userInput = "";
    ...
    KeyboardImpl::KeyboardImpl(Keyboard* kb, const string &text) {
        _owner = kb;
        _text = text;
        _error = "";
        _userInput = "";
    ...
    string &KeyboardImpl::GetInput(void) {
        return (_userInput);
    }

    string &KeyboardImpl::GetMessage(void) {
        return (_text);
    }

    string &KeyboardImpl::GetTitle(void) {
        return _title;
    }

    void KeyboardImpl::SetError(string &error) {
        _errorMessage = true;
        _error = error;
    }
    ...
    void KeyboardImpl::Populate(const vector<CustomIcon> &input, bool resetScroll) {
        ...
        for (const CustomIcon &ico : input) {
            ...
            if (ico.sizeX != 30 || ico.sizeY != 30)
                tks = new TouchKeyString(CustomIcon(Icon::DefaultCustomIcon.pixArray, Icon::DefaultCustomIcon.sizeX, Icon::DefaultCustomIcon.sizeY, ico.isEnabled), box, true);
    ...
    void KeyboardImpl::_ProcessEvent(Event &event) {
        ...
        if (!keyPressIntended && _onKeyboardEvent != nullptr && _owner != nullptr && (event.type == Event::KeyDown || event.type == Event::KeyPressed || event.type == Event::KeyReleased)) {
            ...
            if (event.type == Event::KeyPressed)
                _KeyboardEvent.type = KeyboardEvent::EventType::KeyPressed;

            else if (event.type == Event::KeyDown)
                _KeyboardEvent.type = KeyboardEvent::EventType::KeyDown;

            else if (event.type == Event::KeyReleased)
                _KeyboardEvent.type = KeyboardEvent::EventType::KeyReleased;
    ...
    void KeyboardImpl::_UpdateScroll(float delta, bool ignoreTouch) {
        ...
        if (_displayScrollbar) {
            ...
            if (abs(_inertialVelocity) < INERTIA_THRESHOLD)
                _inertialVelocity = 0.f;
    ...
    void KeyboardImpl::_Update(float delta) {
        ...
        else _UpdateScroll(delta, false);
    }
    ...
    void KeyboardImpl::_DigitKeyboard(vector<TouchKey> &keys) {
    ...
    void KeyboardImpl::_UpdateScrollInfos(void) {
        ...
        if (strWidth > 260.f) {
            if (before > 140.f)
                _offset = before + min(after, 140.f) - 260.f;
    ...
    bool KeyboardImpl::_CheckKeys(void) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (ret != -1) {
                ...
                if (ret == 0x12345678) {
                    if ((_layout == DECIMAL && _userInput.size() >= 18) || (_layout == QWERTY && _max && Utils::GetSize(_userInput) >= _max))
                        return (false);
                    ...
                    if (_KeyboardEvent.codepoint == 0x00B1) {
                        if (_userInput[0] == '-') {
                            _ScrollDown();
        ...
        return (false);
        _backspacePressed:
        string &&right = _userInput.substr(_cursorPositionInString);
        ...
        if (_isIconKeyboard) {
            if (_manualKey == -1) {
                if (key & (Key::Down | Key::Left)) {
                    ...
                    int tempKey = _displayScrollbar ? min((int)_strKeys.size() - 1, _currentPosition + 20) : _strKeys.size() - 1;
