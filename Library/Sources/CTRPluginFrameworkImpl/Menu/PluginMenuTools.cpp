...
namespace CTRPluginFramework {
    ...
    static void Shutdown(void) {
        if (ConfirmBeforeProceed("power-off")) {
    ...
    string CurrentTime() {
        ...
        currentTime <<  "Clock: " << std::setw(2) << std::setfill('0') << hours << ":" << std::setw(2) << minutes << ":" << std::setw(2) << seconds;

        return currentTime.str();
    }

    static const char repo[] = "github.com/biometrix79";
    ...
    void PluginMenuTools::_RenderBottom(void) {
        ...
        const char *info[2] = {InternetAccess().c_str(), CurrentTime().c_str()};

        {
            ...
            int posY2 = 30;
            int posY3 = 45;
            // Renderer::DrawString(nextText, (320 - Renderer::LinuxFontSize(repo)) / 2, bottom, blank);
            ...
            Renderer::DrawString("Repository: github.com/biometrix76", (320 - Renderer::LinuxFontSize("Repository: github.com/biometrix76")) / 2, posY2, Color::Gainsboro);
            Renderer::DrawString(info[1], (320 - Renderer::LinuxFontSize(info[1])) / 2, posY3, Color::Gray);
            ...
            // for (unsigned int i = 0; i < sizeof(info) / sizeof(info[0]); i ++)
                // Renderer::DrawString(info[i], (320 - Renderer::LinuxFontSize(info[i])) / 2, posY2, (i == 0 ? Color::Gainsboro : Color::Gray));
        }
