...
namespace CTRPluginFramework {
    ...
    void PluginMenuExecuteLoop::Add(MenuEntryImpl *entry) {
        ...
        if (entry->_flags.isRadio && id != -1) {
            g_radioId = id;
            vector.erase(std::remove_if (vector.begin(), vector.end(), [](MenuEntryImpl *e) {
    ...
    bool PluginMenuExecuteLoop::operator()(void) {
        ...
        if (needToRemove) {
            _builtinEnabledList.erase(std::remove_if (_builtinEnabledList.begin(), _builtinEnabledList.end(), [](MenuEntryImpl *e) {
