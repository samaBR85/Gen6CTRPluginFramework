...
namespace CTRPluginFramework {
    ...
    bool PluginMenuActionReplay::operator()(EventList &eventList, const Time &delta) {
        ...
        if (_duplicateBtn())
            _DuplicateBtn_OnClick();

        if (_trashBtn())
            _TrashBtn_OnClick();

        if (_openFileBtn())
            _OpenFileBtn_OnClick();
    ...
    void PluginMenuActionReplay::_TrashBtn_OnClick(void) {
        ...
        if (!(MessageBox(CenterAlign("Do you really want to delete this code?"), DialogType::DialogYesNo, ClearScreen::Both)()))
            return;
    ...
    void PluginMenuActionReplay::_OpenFileBtn_OnClick(void) {
        ...
        if (MessageBox(CenterAlign("Save all changes to current file?"), DialogType::DialogYesNo, ClearScreen::Both)())
            SaveCodes();
        ...
        if (root->ItemsCount() > 0) {
            if (MessageBox(CenterAlign("Clear current code list?"), DialogType::DialogYesNo, ClearScreen::Both)()) {
