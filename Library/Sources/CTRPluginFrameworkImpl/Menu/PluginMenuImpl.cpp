...
namespace CTRPluginFramework {
    ...
    void PluginMenuImpl::Callback(CallbackPointer callback) {
        ...
        if (_callbacksTrashBin.size()) {
            auto it = find(_callbacksTrashBin.begin(), _callbacksTrashBin.end(), callback);

            if (it != _callbacksTrashBin.end())
                _callbacksTrashBin.erase(it);
        }

        if (find(_callbacks.begin(), _callbacks.end(), callback) == _callbacks.end())
    ...
    void PluginMenuImpl::RemoveCallback(CallbackPointer callback) {
        ...
        if (find(_callbacksTrashBin.begin(), _callbacksTrashBin.end(), callback) == _callbacksTrashBin.end())
    ...
    int PluginMenuImpl::Run(void) {
        ...
        while (_pluginRun) {
            ...
            while ((_isOpen && openManager.PollEvent(event)) || (!_isOpen && closedManager.PollEvent(event)) || _forceOpen) {
                ...
                if (event.type == Event::KeyPressed && inputClock.HasTimePassed(Milliseconds(500)) && (!Preferences::IsEnabled(Preferences::UseFloatingBtn) || _isOpen) && Controller::GetKeysDown() != SystemImpl::RosalinaHotkey) {
            ...
            if (_isOpen) {
                ...
                if (_callbacksTrashBin.size()) {
                    ...
                    if (_callbacksTrashBin.size()) {
                        _callbacks.erase(remove_if (_callbacks.begin(), _callbacks.end(), [](CallbackPointer cb) {
    ...
    void PluginMenuImpl::GetRegionsList(vector<Region> &list) {
        if (_runningInstance != nullptr)
