...
namespace CTRPluginFramework {
        ...
        if (event.type == Event::KeyPressed) {
            switch (event.key.code) {
                ...
                case Key::B: {
                    ...
                    else Window::BottomWindow.Close();
    ...
    static const char link[] = "github.com/biometrix79";

    s32 HexEditor::_RenderTop(void *arg) {
        HexEditor *obj = __g_hexEditor;
        Context &_ctx = obj->_ctx;
        Renderer::SetTarget(TOP);
        const Color &maintextcolor = Preferences::Settings.MainTextColor;
        int posY = 61;
        Window::TopWindow.Draw("                            HexEditor"); // 28 spaces
        Renderer::DrawRect(44, posY, 66, 20, Color::Silver);
        posY += 5;
        Renderer::DrawString(obj->_ctx._cursorAddressStr, 54, posY, Color::Black);
        posY += 6;
        Renderer::DrawRect(44, posY, 66, 100, Color::Silver);
    ...
    void HexEditor::_CreateCheat(void) {
        ...
        if (_ctx._flags & InvalidSrc) {
            MessageBox(CenterAlign("Invalid address, now aborting..."), DialogType::DialogOk, ClearScreen::Both)();
            return;
        }
    ...
    void HexEditor::_BrowseHistory(void) {
        if (_history.empty()) {
            MessageBox(CenterAlign("Cannot go back. History is empty."), DialogType::DialogOk, ClearScreen::Both)();
            return;
        }
    ...
    bool HexEditor::Goto(u32 address, bool updateCursor) {
        ...
        else _ctx._flags |= DirtyCursorAddress;
    ...
    void HexEditor::_JumpTo(int mode) {
        ...
        if (mode == JumpToValue) {
            ...
            if (!(MessageBox(CenterAlign(Utils::Format("Do you want to jump to: 0x%08X?", address)), DialogType::DialogYesNo, ClearScreen::Both))())
                return;
        }
    ...
    void HexEditor::IView::ProcessEventsCommon(Context& _ctx, Event& event, s16 maxX) {
        if (event.type == Event::EventType::KeyPressed || (event.type == Event::KeyDown && _ctx._clock.HasTimePassed(_ctx._latency))) {
            _ctx._clock.Restart();
            u32 &flags = _ctx._flags;
    ...
    void HexEditor::ByteView::Draw(void) {
        ...
        // Draw value header
        Renderer::DrawRect(111, posY, 178, 20, Color::Gray);
        posY += 5;
        ...
        // Characters header
        posY -= 21;
        Renderer::DrawRect(290, posY, 66, 20, Color::Gray);

        // Characters body
        posY += 21;
        Renderer::DrawRect(290, posY, 66, 100, Color::Gray);

        // Draw Cursor
        Renderer::DrawRect(_ctx._cursorPosX, _ctx._cursorPosY, 7, 10, Color::Gray);
    ...
    void HexEditor::ByteView::UpdateView(void) {
        ...
        if (flags & DirtyViewCache) {
            ...
            for (Item &item: _ctx._items) {
                bool red = false;
                u8 *b = &item.b[0];
                string &value = item.valueCache;
                string &data = item.dataCache;
                value.clear();
                data.clear();
        ...
        if (flags & DirtyCursorPos) {
            ...
            // posX
            {
                ...
                _ctx._cursorPosX = startX + bytes * byteWidth + ((_ctx._cursorX & 1) ? 6 : 0) + ((_ctx._cursorX >> 3) > 0 ? 12 : 0);
            }
    ...
    void HexEditor::IntegerView::Draw(void) {
        ...
        // Draw value header
        Renderer::DrawRect(111, posY, 245, 20, Color::Gray);
        posY += 21;
    ...
    void HexEditor::IntegerView::UpdateView(void) {
        ...
        if (flags & DirtyCursor) {
            ...
            u32 targetX = (target - (target >> shift << shift)) << 1;

            _ctx._cursorX = targetX >> 3 << 3;
            _ctx._cursorX += 6 - (targetX - _ctx._cursorX);
            _ctx._cursorY = target >> shift;
        }
        ...
        if (flags & DirtyCursorPos) {
            ...
            // posX
            {
                ...
                _ctx._cursorPosX = startX + bytes * byteWidth + space + ((_ctx._cursorX & 1) ? 6 : 0);
            }
    ...
    void HexEditor::AsmView::Draw(void) {
        ...
        // Draw value header
        Renderer::DrawRect(111, posY, 58, 20, Color::Gray);
        posY += 21;
        Renderer::DrawRect(111, posY, 58, 100, Color::White);

        posY -= 21;
        // Characters header
        Renderer::DrawRect(170, posY, 186, 20, Color::Gray);
        // Characters body
        posY += 21;
        Renderer::DrawRect(170, posY, 186, 100, Color::Gray);
    ...
    void HexEditor::AsmView::UpdateView(void) {
        ...
        if (flags & DirtyCursorPos) {
            ...
            // posX
            {
                ...
                u32 byteWidth = 12;
                u32 bytes = _ctx._cursorX >> 1;
                _ctx._cursorPosX = startX + bytes * byteWidth + ((_ctx._cursorX & 1) ? 6 : 0);
            }
