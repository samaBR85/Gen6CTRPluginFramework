...
namespace CTRPluginFramework {
    ...
    string Preferences::CheatsFile;
    string Preferences::ScreenshotPath;
    string Preferences::ScreenshotPrefix;
    ...
    static const char *g_signature = "MPF\0\0";
    ...
    int Preferences::OpenConfigFile(File &settings, Header &header) {
        if (File::Open(settings, "Data.bin") == 0 && settings.GetSize() > 0) {
            // Check version
            int res = 0;
            ...
            // Check file
            if (header.size != settings.GetSize() || !equal(g_signature, g_signature + 8, header.sig))
                return -1;
    ...
    void Preferences::LoadSavedEnabledCheats(void) {
        ...
        if (_cheatsAlreadyLoaded) {
            MessageBox(CenterAlign("Error, cheats have been already loaded!"), DialogType::DialogOk, ClearScreen::Both)();
            return;
        }
    ...
    void Preferences::LoadSavedFavorites(void) {
        ...
        if (_favoritesAlreadyLoaded) {
            MessageBox(CenterAlign("Favorites already loaded!"), DialogType::DialogOk, ClearScreen::Both)();
            return;
        }
