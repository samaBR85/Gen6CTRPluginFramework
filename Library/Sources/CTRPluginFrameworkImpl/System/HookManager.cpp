...
HookManager::HookManager(void) {
    HookWrapper zero;
    fill(_hookWrappers, _hookWrappers + MAX_HOOKS, zero);
...
extern "C"  void __hook__CheckTlsValue(u32 value) {
    if (value && (value < 0x06000000 || value > 0x07000000)) {
        ...
        REG32(0x10202204) = 0;
        const char *str = "Contact: discord.gg/z4ZMh27";
...
static __attribute__((noinline)) void GenerateAsm(AsmWrapper &asmWrapper, HookContext &ctx) {
    ...
    *code++ = 0xE92D400F;
    *code++ = 0xE51F0010;
    *code++ = ARMBranchLink(&asmWrapper.code[2], (void*)SetHookContextInTLS);
    *code++ = 0xE8BD400F;
    ...
    if (flags & MITM_MODE) {
        ...
        *code++ = ctx.callbackAddress;
        ...
        // If instruction is branch with link
        if (op == 0xEB)
        ...
        else *code = ctx.overwrittenInstr;
...
HookResult HookManager::ApplyHook(HookContext &ctx) {
    ...
    // Update HookWrapper
    hookWrapper.jumpCode = 0xE51FF004;
