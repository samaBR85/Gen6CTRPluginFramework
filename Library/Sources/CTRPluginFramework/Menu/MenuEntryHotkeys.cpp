...
namespace CTRPluginFramework {
    ...
    // Function to assign a single hotkey to a menu entry
    MenuEntry *HotkeyEntry(MenuEntry *entry, const Hotkey &hotkey) {
        // Check if the entry is valid
        if (entry != nullptr) {
            // Add the hotkey to the entry's hotkey list
    ...
    // Function to assign multiple hotkeys to a menu entry
    MenuEntry *HotkeyEntry(MenuEntry *entry, const vector<Hotkey> &hotkeys) {
        // Check if the entry is valid
        if (entry != nullptr) {
            // Iterate through the vector of hotkeys and add each to the entry's hotkey list
