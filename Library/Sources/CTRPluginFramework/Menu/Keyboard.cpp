...
namespace CTRPluginFramework {
    ...
    bool stou64(string &input, u64 &res) {
        const char *str = input.c_str();
        bool neg = false;
    ...
    void *ConvertToU8(string &input, bool isHex) {
        static u8 temp = 0;
    ...
    void *ConvertToU16(string &input, bool isHex) {
        static u16 temp = 0;
    ...
    void *ConvertToU32(string &input, bool isHex) {
        static u32 temp = 0;
    ...
    void *ConvertToU64(string &input, bool isHex) {
        static u64 temp = 0;
    ...
    void *ConvertToFloat(string &input, bool isHex) {
        static float temp = 0;

        if (input.length() > 0 && !(input.length() == 1 && input[0] == '-'))
            temp = static_cast<float>(stof(input, nullptr));
    ...
    void *ConvertToDouble(string &input, bool isHex) {
        static double temp = 0;

        if (input.length() > 0 && !(input.length() == 1 && input[0] == '-'))
            temp = static_cast<double>(stod(input, nullptr));
    ...
    Keyboard::Keyboard(const string &text) :
    ...
    Keyboard::Keyboard(const string &text, const vector<string> &options) :
        _keyboard(new KeyboardImpl(this, text))
    ...
    Keyboard::Keyboard(const vector<string> &options) :
    ...
    void Keyboard::Populate(const vector<string> &input, bool resetScroll) {
        _keyboard->Populate(input, resetScroll);
        _isPopulated = true;
    }

    void Keyboard::Populate(const vector<CustomIcon> &input, bool resetScroll) {
        _keyboard->Populate(input, resetScroll);
        _isPopulated = true;
    }
    ...
    int Keyboard::Open(string &output) const {
        if (SystemImpl::IsSleeping())
            return (-2);
    ...
    int Keyboard::Open(string &output, const string &start) const {
        if (SystemImpl::IsSleeping())
            return (-2);
    ...
    string &Keyboard::GetInput(void) const {
        return (_keyboard->GetInput());
    }

    string &Keyboard::GetMessage(void) const {
        return (_keyboard->GetMessage());
    }

    void Keyboard::SetError(string error) const {
    ...
    int Keyboard::Setup(const string &name, bool canAbort, bool isHex, const int maxLength, u8 &output, u8 start) {
    ...
    int Keyboard::Setup(const string &name, bool canAbort, bool isHex, const int maxLength, u16 &output, u16 start) {
        Sleep(Milliseconds(100));
        Keyboard keyboard(name);
        keyboard.CanAbort(canAbort);
        keyboard.IsHexadecimal(isHex);
    ...
    int Keyboard::Setup(const string &name, bool canAbort, bool isHex, const int maxLength, u32 &output, u32 start) {
        Sleep(Milliseconds(100));
        Keyboard keyboard(name);
        keyboard.CanAbort(canAbort);
        keyboard.IsHexadecimal(isHex);
    ...
    int Keyboard::Setup(const string &name, bool canAbort, const vector<string> &options, int &index) {
        Sleep(Milliseconds(100));
        Keyboard keyboard(name);
        keyboard.CanAbort(canAbort);
